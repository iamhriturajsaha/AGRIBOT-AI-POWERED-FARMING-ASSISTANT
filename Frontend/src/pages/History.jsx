    ...
    <div className="max-w-6xl mx-auto pb-10 pt-4">
      <div className="mb-10 flex items-center">
        <motion.div 
          ...
          className="inline-flex w-16 h-16 rounded-2xl bg-neon-blue/10 border border-neon-blue/20 items-center justify-center mr-6"
        >
          <Clock className="w-8 h-8 text-neon-blue" />
        </motion.div>
        <div>
          <h1 className="text-4xl font-display font-bold text-white mb-2">Farm Health Log</h1>
          <p className="text-gray-400">Track and review all previous crop disease predictions.</p>
        </div>
      </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-white/10 bg-white/5">
                    ...
                    <th className="px-6 py-4 font-medium text-gray-300">Image</th>
                    <th className="px-6 py-4 font-medium text-gray-300">Crop & Diagnosis</th>
                    <th className="px-6 py-4 font-medium text-gray-300">Confidence</th>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="w-12 h-12 rounded-lg bg-black/50 overflow-hidden border border-white/10 flex items-center justify-center">
                            {record.image ? (
                        ...
                        <td className="px-6 py-4">
                          <div className="font-medium text-white">{crop}</div>
                          <div className="text-sm text-gray-400">{diseaseClean}</div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center">
                            <span className="text-white font-medium mr-2">{(record.confidence * 100).toFixed(1)}%</span>
                            <div className="w-16 h-1.5 bg-white/10 rounded-full overflow-hidden">
                              <div 
                                className={`h-full ${isHealthy ? 'bg-neon-green' : 'bg-red-500'}`} 
                        ...
                        <td className="px-6 py-4">
                          {isHealthy ? (
                            ...
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-500/10 text-red-400 border border-red-500/20">
                              <AlertTriangle className="w-3 h-3 mr-1" /> Detected
                            </span>
                          )}
                        </td>
                      </motion.tr>
