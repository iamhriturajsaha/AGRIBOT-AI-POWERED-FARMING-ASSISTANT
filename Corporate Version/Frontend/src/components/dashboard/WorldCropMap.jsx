    ...
    <div className="w-full relative bg-transparent">
      <div className="absolute top-6 left-6 z-10 pointer-events-none">
         <div className="flex items-center text-white mb-1">
            <Globe2 className="w-6 h-6 mr-3 text-agri-green" />
            <h3 className="text-2xl font-bold font-display tracking-wide uppercase">Global Satellite Telemetry</h3>
         </div>
         <p className="text-slate-400 text-sm font-medium ml-9">Multi-continental crop distribution and structural health</p>
         
         <div className="ml-9 mt-4 flex items-center space-x-4">
           <div className="flex items-center"><span className="w-2 h-2 rounded-full bg-green-500 mr-2 shadow-[0_0_4px_rgba(34,197,94,0.5)]"></span><span className="text-xs font-semibold text-slate-400">Optimal</span></div>
           <div className="flex items-center"><span className="w-2 h-2 rounded-full bg-amber-400 mr-2 shadow-[0_0_4px_rgba(251,191,36,0.5)]"></span><span className="text-xs font-semibold text-slate-400">Warning</span></div>
           <div className="flex items-center"><span className="w-2 h-2 rounded-full bg-red-500 mr-2 shadow-[0_0_4px_rgba(239,68,68,0.5)] animate-pulse"></span><span className="text-xs font-semibold text-slate-400">Critical</span></div>
         </div>
      </div>
      
      {/* Instruction UI */}
      <div className="absolute bottom-4 right-4 z-10 bg-slate-900/40/80 backdrop-blur-md px-3 py-1.5 rounded-md border border-slate-800 text-xs font-bold text-slate-400 tracking-wider pointer-events-none">
         [ SCROLL TO ZOOM / DRAG TO PAN ]
      </div>

      <div className="w-full h-[500px] md:h-[600px] overflow-hidden rounded-b-xl border-t border-slate-100 cursor-crosshair">
        <ComposableMap projection="geoMercator" className="w-full h-full outline-none" projectionConfig={{ scale: 130 }}>
          <ZoomableGroup zoom={1} center={[0, 20]} minZoom={1} maxZoom={8}>
                ...
                geographies.map((geo) => (
                  <Geography
                    ...
                    style={{
                      ...
                      hover: { fill: "rgba(34, 197, 94, 0.1)", stroke: "#22c55e", strokeWidth: 1, outline: "none" },
            ...
            {markers.map(({ name, coordinates, markerOffset, crop, health }) => {
              const markerColor = health === 'Optimal' ? '#22c55e' : health === 'Warning' ? '#f59e0b' : '#ef4444';
              return (
                <Marker key={name} coordinates={coordinates}>
                  <circle 
                    ...
                    style={{ filter: `drop-shadow(0px 0px 4px ${markerColor})` }}
                  />
                  {health === 'Critical' && (
                    <circle r={12} fill="transparent" stroke={markerColor} strokeWidth="1.5" className="animate-ping" />
                  )}
                  <text
                    textAnchor="middle"
                    y={markerOffset}
